...
export function PersonnelGrid() {
    ...
    const fetchMatrix = () => {
        fetch('http://localhost:3000/api/personnel/matrix')
            .then(res => res.json())
    ...
    const updateStatus = async (userId: string, date: string, newStatus: string, note?: string) => {
        await fetch('http://localhost:3000/api/personnel/availability', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ user_id: userId, date, status: newStatus, note }),
        });
    ...
    const handleAddRigger = async () => {
        await fetch('http://localhost:3000/api/personnel', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(newRigger),
        });
        setOpen(false);
                        ...
                        return (
                            ...
                            >
                                ...
                                <span className={`text-xs font-bold mt-0.5 leading-none
                                    ${today ? 'text-blue-600' : wknd ? 'text-muted-foreground' : 'text-foreground'}
                                `}>
                                    {format(day, 'd')}
                                </span>
                            </div>
