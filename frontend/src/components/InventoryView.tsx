...
import { getParachutes, addParachute, bulkAddParachutes } from '@/lib/mockData';
import type { Parachute } from '@/lib/mockData';

export function InventoryView() {
  const [inventory, setInventory] = useState<Parachute[]>([]);
  const [open, setOpen] = useState(false);
  const [bulkOpen, setBulkOpen] = useState(false);
  const [newChute, setNewChute] = useState<{ serial_number: string; model: string; last_pack_date: string; category: 'main' | 'reserve' }>({ serial_number: '', model: '', last_pack_date: '', category: 'main' });
  const [bulkForm, setBulkForm] = useState<{ count: number; model: string; category: 'main' | 'reserve'; last_pack_date: string; serial_prefix: string }>({ count: 10, model: 'T-11', category: 'main', last_pack_date: '', serial_prefix: '' });

  const fetchInventory = () => {
    setInventory(getParachutes());
  };
  ...
  const handleAddChute = () => {
    addParachute({
      ...newChute,
      category: newChute.category as 'main' | 'reserve',
  ...
  const handleBulkAdd = () => {
    bulkAddParachutes({
      ...bulkForm,
      category: bulkForm.category as 'main' | 'reserve',
  ...
  const getStatusColor = (status: string, days: number | null) => {
    ...
    if (status === 'in_process') return 'bg-blue-100 text-blue-800 border-blue-200';
