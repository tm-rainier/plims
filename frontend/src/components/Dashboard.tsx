...
function KPICard({ title, value, icon, description, accent }: KPICardProps) {
  return (
...
interface DashboardStats {
  ...
  recent_activity: { rank: string; name: string; pack_count: number; date: string }[];
}
...
export function Dashboard() {
  ...
  const readyPct = stats.mission_ready_percent;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
        <KPICard
          ...
          description={`${stats.ready_chutes} of ${stats.total_chutes} chutes`}
          accent={readinessAccent(readyPct)}
        />
        <KPICard
          title="Riggers"
          value={stats.riggers_on_floor}
          icon={<Users className="h-4 w-4 text-muted-foreground" />}
          description={`${stats.total_riggers - stats.riggers_on_floor} unavailable`}
        />
        <KPICard
          title="30-Day Cap."
          value={stats.next_30_day_capacity.toLocaleString()}
            ...
            <CardContent>
              <div className="space-y-3">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-semibold text-green-700">{stats.ready_chutes}</span>
                </div>
                <div className="h-2 rounded-full bg-muted overflow-hidden">
                  <div
                    className="h-full bg-green-500 rounded-full transition-all"
                    style={{ width: `${stats.total_chutes > 0 ? (stats.ready_chutes / stats.total_chutes) * 100 : 0}%` }}
                  />
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Total chutes</span>
                  <span className="font-semibold">{stats.total_chutes}</span>
                </div>
                <div className="flex items-center justify-between text-sm">
                  <span className="text-muted-foreground">Not ready</span>
                  <span className="font-semibold text-amber-600">
                    {stats.total_chutes - stats.ready_chutes}
                  </span>
                </div>
              </div>
            </CardContent>
