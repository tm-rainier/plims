...
function getEventsForMonth(events: Event[], month: Date) {
...
function EventChip({ event, feasible, onClick }: { event: Event; feasible: boolean; onClick: () => void }) {
    return (
        <button
            onClick={onClick}
...
export function CalendarView() {
    ...
    const fetchEvents = async () => {
        ...
        const feasibilityEntries = await Promise.all(
            data.map(async (event: Event) => {
                try {
                    const fRes = await fetch(`http://localhost:3000/api/events/${event.id}/feasibility`);
                    const fData = await fRes.json();
                    return [event.id, fData] as const;
                } catch (err) {
                    console.error(`Failed to fetch feasibility for event ${event.id}`, err);
                    return null;
                }
            })
        );
        ...
        for (const entry of feasibilityEntries) {
            if (entry) feasibilityData[entry[0]] = entry[1];
        }
    ...
    useEffect(() => {
        (async () => {
            try { await fetchEvents(); } catch (err) { console.error(err); }
        })();
    }, []);
    ...
    const handleEditEvent = async () => {
        if (!editingId) return;
        await fetch(`http://localhost:3000/api/events/${editingId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(newEvent)
        });
